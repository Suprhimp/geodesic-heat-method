...
import { indexElements } from '../GeometryCore/mesh';
...
export class HeatMethod {
   ...
   */
  computeVectorField(u: DenseMatrix) {
    let X: any = {};
    for (let f of this.geometry.mesh.faces) {
      ...
      for (let h of f.adjacentHalfedges()) {
        let i = this.vertexIndex[h!.prev!.vertex!.index];
        let ui = u.get(i, 0);
   ...
   */
  subtractMinimumDistance(phi: DenseMatrix) {
    let min = Infinity;
